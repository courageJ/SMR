...
void init_index() {
    ...
    index_buffer.buffered_fingerprints = g_hash_table_new_full(g_int64_hash,
            (GEqualFunc)g_fingerprint_equal, NULL, NULL);
...
void index_update(GHashTable *features, int64_t id){
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        kvstore_update((unsigned char *)key, id);
    }
}

void index_delete(fingerprint *fp, int64_t id){
	kvstore_delete((unsigned char *)fp, id);
}
...
void index_check_buffer(struct segment *s) {
    ...
    for (; iter != end; iter = g_sequence_iter_next(iter)) {
        ...
        if((CHECK_CHUNK(c, CHUNK_DUPLICATE) && c->id == TEMPORARY_ID)
             ...
             */
            GQueue* bq = (GQueue*)g_hash_table_lookup(index_buffer.buffered_fingerprints, &c->fp);
            assert(bq);
            struct indexElem* be = (indexElem*)g_queue_peek_head(bq);
...
int index_update_buffer(struct segment *s){
    ...
    for (; iter != end; iter = g_sequence_iter_next(iter)) {
        ...
        struct indexElem* e = (indexElem*)g_queue_pop_head(bq);
        if(g_queue_get_length(bq) == 0){
            ...
            for(j=0; j<num; j++){
                struct indexElem* ie = (indexElem*)g_queue_peek_nth(bq, j);
                ie->id = c->id;
            }
        }
        free(e);
