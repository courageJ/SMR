...
void har_monitor_update(containerid id, int32_t size) {
	TIMER_DECLARE(1);
	TIMER_BEGIN(1);
	struct containerRecord* record = (containerRecord* )g_hash_table_lookup(
...
void close_har() {
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		if((1.0*cr->size/(CONTAINER_SIZE - CONTAINER_META_SIZE))
			...
			g_sequence_insert_sorted(seq, cr, (GCompareDataFunc)g_record_cmp, NULL);
		}
	}
	...
	while(destor.rewrite_har_rewrite_limit < 1
		...
		struct containerRecord* r = (containerRecord* )g_sequence_get(iter);
	...
	while(sparse_iter != g_sequence_get_end_iter(seq)){
		struct containerRecord* r =(containerRecord* ) g_sequence_get(sparse_iter);
		fprintf(fp, "%lld %d\n", r->cid, r->size);
