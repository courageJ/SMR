...
static int container_record_check_id(void *a1, void *id1) {
		struct containerRecord* a = (containerRecord*)a1;
...
void restore_aware_update(containerid id, int32_t chunklen) {
	...
	struct containerRecord* record = (containerRecord*)lru_cache_lookup(monitor.cache, &id);
	if (!record) {
