...
struct structstoragebuffer storage_buffer;
/*
extern struct {
	GMutex mutex;
...
static void* filter_thread(void *arg) {
    ...
    while (1) {
        struct chunk* c = (chunk*)sync_queue_pop(rewrite_queue);
        ...
        c = (chunk*)sync_queue_pop(rewrite_queue);
        ...
        GHashTable *recently_rewritten_chunks = g_hash_table_new_full(g_int64_hash,
        		(GEqualFunc)g_fingerprint_equal, NULL, (GDestroyNotify)free_chunk);
        GHashTable *recently_unique_chunks = g_hash_table_new_full(g_int64_hash,
        			(GEqualFunc)g_fingerprint_equal, NULL, (GDestroyNotify)free_chunk);

        pthread_mutex_lock((pthread_mutex_t*)(&index_lock.mutex));
        ...
        for (; iter != end; iter = g_sequence_iter_next(iter)) {
            ...
            if(CHECK_CHUNK(c, CHUNK_DUPLICATE) && c->id == TEMPORARY_ID){
            	struct chunk* ruc = (chunk*)g_hash_table_lookup(recently_unique_chunks, &c->fp);
            	assert(ruc);
            	c->id = ruc->id;
            }
            struct chunk* rwc = (chunk*)g_hash_table_lookup(recently_rewritten_chunks, &c->fp);
            if(rwc){
            	c->id = rwc->id;
            ...
            if (!CHECK_CHUNK(c, CHUNK_DUPLICATE) 
                ...
                if (storage_buffer.container_buffer == NULL){
                	...
                	if(destor.index_category[1] == INDEX_CATEGORY_PHYSICAL_LOCALITY)
                		storage_buffer.chunks = g_sequence_new((GDestroyNotify)free_chunk);
                }
        ...
        for (; iter != end; iter = g_sequence_iter_next(iter)) {
        	...
        	if(r == NULL){
        		...
        		r = new_file_recipe_meta((char *)c->data);
        ...
        if(index_lock.wait_threshold > 0 && full == 0){
        	pthread_cond_broadcast((pthread_cond_t*)(&index_lock.cond));
        }
        TIMER_END(1, jcr.filter_time);
        pthread_mutex_unlock((pthread_mutex_t*)(&index_lock.mutex));
