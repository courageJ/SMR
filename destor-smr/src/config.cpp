...
void load_config_from_string(sds config) {
	...
	for (i = 0; i < totlines; i++) {
		...
		if (strcasecmp(argv[0], "working-directory") == 0 && argc == 2) {
			...
			if (strcasecmp(argv[1], "no") == 0)
				...
				destor.rewrite_algorithm[0] = REWRITE_CAPPING;
					...
					|| strcasecmp(argv[1], "SMR") == 0)
				destor.rewrite_algorithm[0] = REWRITE_SMR;//courageJ
			else if (strcasecmp(argv[1], "NED") == 0
					|| strcasecmp(argv[1], "ned") == 0)
				destor.rewrite_algorithm[0] = REWRITE_NED;//courageJ
			else {
		...
		} else if (strcasecmp(argv[0], "rewrite-smr-level") == 0
				&& argc == 2) {
			destor.rewrite_smr_level = atoi(argv[1]);
			
		} else if (strcasecmp(argv[0], "rewrite-ned-level") == 0
				&& argc == 2) {
			destor.rewrite_ned_level = atoi(argv[1]);
