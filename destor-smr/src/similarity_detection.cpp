...
static void top_segment_select(GHashTable* features) {
	...
	while (g_hash_table_iter_next(&iter, &key, &value)) {
		...
		segmentid *ids = (segmentid *)kvstore_lookup((unsigned char *)key);
		if (ids) {
			...
			for (i = 0; i < destor.index_value_length; i++) {
				if (ids[i] == TEMPORARY_ID)
					break;
				struct segment* s = (segment *)g_hash_table_lookup(similar_segments, &ids[i]);
				if (!s) {
					s = new_segment_full();
					s->id = ids[i];
					g_hash_table_insert(similar_segments, &s->id, s);
				}
				char* feature = (char* )malloc(destor.index_key_size);
	...
	if (g_hash_table_size(similar_segments) != 0) {
		...
		while (g_hash_table_iter_next(&iter, &key, &value)) {
			...
			struct segment* s = (segment*)value;
			NOTICE("candidate segment %lld with %d shared features", s->id,
					g_hash_table_size(s->features));
			g_sequence_insert_sorted(seq, s, (GCompareDataFunc)g_segment_cmp_feature_num, NULL);
		}
		...
		for (i = 0; i < num; i++) {
			/* Get the top segment */
			struct segment *top = (segment*)g_sequence_get(
			...
			g_sequence_foreach(seq, (GFunc)features_trim, top);
			g_sequence_sort(seq, (GCompareDataFunc)g_segment_cmp_feature_num, NULL);
		}
		g_sequence_free(seq);
...
extern struct structstoragebuffer storage_buffer;
...
void index_lookup_similarity_detection(struct segment *s){
	...
	for (; iter != end; iter = g_sequence_iter_next(iter)) {
		struct chunk* c = (chunk*)g_sequence_get(iter);
		 ...
		 */
		GQueue *tq = (GQueue *)g_hash_table_lookup(index_buffer.buffered_fingerprints, &c->fp);
		if (!tq) {
			tq = g_queue_new();
		} else if (!CHECK_CHUNK(c, CHUNK_DUPLICATE)) {
			struct indexElem *be = (indexElem *)g_queue_peek_head(tq);
			c->id = be->id;
			SET_CHUNK(c, CHUNK_DUPLICATE);
		}
		...
		if(destor.index_category[0] == INDEX_CATEGORY_EXACT
			...
			if (!CHECK_CHUNK(c, CHUNK_DUPLICATE)) {
				...
				int64_t* ids = kvstore_lookup((unsigned char*)&c->fp);
				if(ids){
					index_overhead.lookup_requests++;
