...
static void* chunk_thread(void *arg) {
	int leftlen = 0;
	int leftoff = 0;
	unsigned char *leftbuf =(unsigned char *) malloc(DEFAULT_BLOCK_SIZE + destor.chunk_max_size);

	unsigned char *zeros =(unsigned char *) malloc(destor.chunk_max_size);
	bzero(zeros, destor.chunk_max_size);
	unsigned char *data =(unsigned char *) malloc(destor.chunk_max_size);
	...
	while (1) {
		...
		c = (chunk*)sync_queue_pop(read_queue);
