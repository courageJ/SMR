...
static void* append_thread(void *arg) {
	...
	while (1) {
		struct container *c = (container *)sync_queue_get_top(container_buffer);
		if (c == NULL)
			break;
...
static void init_container_meta(struct containerMeta *meta) {
	...
	meta->map = g_hash_table_new_full(g_int_hash,(GEqualFunc) g_fingerprint_equal, NULL,
			free);
}
...
struct container* retrieve_container_by_id(containerid id) {
	...
	if (destor.simulation_level >= SIMULATION_RESTORE) {
		c->data = (unsigned char*)malloc(CONTAINER_META_SIZE);
...
static void * container_meta_duplicate(void *c1) {
//static struct containerMeta* container_meta_duplicate(void *c1) {
	struct container *c = (container *)c1;
...
struct containerMeta* retrieve_container_meta_by_id(containerid id) {
	...
	cm = (containerMeta*)sync_queue_find(container_buffer, container_check_id, &id,
...
static struct metaEntry* get_metaentry_in_container_meta(
	...
	return (metaEntry*)g_hash_table_lookup(cm->map, fp);
}
 ...
 */
int lookup_fingerprint_in_container_meta(void *cm1, void *fp1) {
	struct containerMeta* cm = (containerMeta*) cm1;
		fingerprint *fp = (fingerprint *) fp1;
	return g_hash_table_lookup(cm->map, fp) == NULL ? 0 : 1;
}

int lookup_fingerprint_in_container(void *c1, void *fp1) {
	struct container* c = (container *) c1;
	fingerprint *fp = (fingerprint *) fp1;
...
int container_check_id(void *c1, void *id1) {
	struct container* c = (container *) c1;
	containerid* id = (containerid *) id1;
	return container_meta_check_id(&c->meta, id);
}

int container_meta_check_id(void *cm1, void *id1) {
	struct containerMeta* cm = (containerMeta*) cm1;
	containerid* id = (containerid *) id1;
	return cm->id == *id ? 1 : 0;
}
