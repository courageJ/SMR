...
struct backupVersion* create_backup_version(const char *path) {
	...
	b->metabuf = (char *)malloc(metabufsize);
	b->metabufoff = 0;
	...
	b->recordbuf = (char *)malloc(recordbufsize);
	b->recordbufoff = 0;
...
segmentid append_segment_flag(struct backupVersion* b, int flag, int segment_size){
	...
	if(flag == CHUNK_SEGMENT_START){
		...
		b->segmentbuf = (char *)malloc(b->segmentlen);
		b->segmentbufoff = 0;
	}
...
struct segmentRecipe* new_segment_recipe() {
	...
	sr->id = TEMPORARY_ID;
	sr->kvpairs = g_hash_table_new_full(g_int64_hash, (GEqualFunc)g_fingerprint_equal,
			NULL, free);
	return sr;
}

void free_segment_recipe(void *sr1) {
	struct segmentRecipe* sr = (segmentRecipe *) sr1;
...
int segment_recipe_check_id(void *sr1, void *id1) {
	segmentRecipe* sr = (segmentRecipe *) sr1;
	segmentid *id = (segmentid *)id1;
     return sr->id == *id;
}

int lookup_fingerprint_in_segment_recipe(void *sr1, void *fp1) {
 		segmentRecipe* sr = (segmentRecipe *) sr1;
