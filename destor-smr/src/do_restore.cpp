...
static void* lru_restore_thread(void *arg) {
	...
	while ((c = (chunk*)sync_queue_pop(restore_recipe_queue))) {
		...
		if (destor.simulation_level >= SIMULATION_RESTORE) {
			struct containerMeta *cm = (containerMeta *)lru_cache_lookup(cache, &c->fp);
			if (!cm) {
			...
			struct container *con = (container *)lru_cache_lookup(cache, &c->fp);
			if (!con) {
				VERBOSE("Restore cache: container %lld is missed", c->id);
...
void* write_restore_data(void* arg) {
	...
	while ((c = (chunk *)sync_queue_pop(restore_chunk_queue))) {
		...
		if (CHECK_CHUNK(c, CHUNK_FILE_START)) {
			...
			filepath = sdscat(filepath, (const char*)c->data);
...
void do_restore(int revision, char *path) {
    ...
    do{
        sleep(5);
        fprintf(stderr, "%lld bytes, %d chunks, %d files processed\r", 
                jcr.data_size, jcr.chunk_num, jcr.file_num);
    }while(jcr.status == JCR_STATUS_RUNNING || jcr.status != JCR_STATUS_DONE);
    fprintf(stderr, "%lld bytes, %d chunks, %d files processed\n", 
        jcr.data_size, jcr.chunk_num, jcr.file_num);
