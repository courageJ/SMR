...
void destor_stat() {
	...
	printf("deduplication ratio: %.4f, %.4f\n",(destor.data_size - destor.stored_data_size)/ (double) destor.data_size,((double) destor.data_size) / (destor.stored_data_size));
...
void free_segment(struct segment* s) {
	...
	for(; begin != end; begin = g_sequence_get_begin_iter(s->chunks)){
		free_chunk((chunk *)g_sequence_get(begin));
