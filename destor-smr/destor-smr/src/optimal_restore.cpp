...
static gint g_access_records_cmp_by_first_seqno(struct accessRecords *a,
	...
	int *da = (int *)g_queue_peek_head(a->seqno_queue);
	if (da == NULL)
		return 1;

	int *db = (int *)g_queue_peek_head(b->seqno_queue);
	if (db == NULL)
		return -1;
...
static void optimal_cache_window_fill() {
	...
	if (ids) {
		...
		for (i = 1; i <= k; i++) {
			...
			struct accessRecords* r = (accessRecords*)g_hash_table_lookup(
...
void init_optimal_cache() {
	...
	optimal_cache.access_record_table = g_hash_table_new_full(g_int64_hash, g_int64_equal,
			NULL, (GDestroyNotify)free_access_records);
...
static void optimal_cache_window_slides(containerid id) {
	...
	struct accessRecords *r = (accessRecords *)g_hash_table_lookup(optimal_cache.access_record_table, &id);
	assert(r);
	int *d = (int *)g_queue_pop_head(r->seqno_queue);
	free(d);
...
static struct chunk* optimal_cache_lookup(fingerprint *fp) {
	...
	struct container* con = (container*)lru_cache_lookup(optimal_cache.lru_queue, fp);
...
static int find_kicked_container(void *con1, void *ht1) {
	struct container* con = (container *) con1;
	GHashTable *ht = (GHashTable *)ht1;

	struct accessRecords* r = (accessRecords*)g_hash_table_lookup(ht, &con->meta.id);
	if(r){
		victim = r;
		return 1;
	}
...
static int find_kicked_container_meta(void *cm1, void *ht1) {
	struct containerMeta* cm =  (containerMeta *) cm1;
	GHashTable *ht = (GHashTable *)ht1;

	struct accessRecords* r = (accessRecords*)g_hash_table_lookup(ht, &cm->id);
	if(r){
		victim = r;
		return 1;
	}
...
static void optimal_cache_insert(containerid id) {
	...
	if (lru_cache_is_full(optimal_cache.lru_queue)) {
		...
		g_sequence_sort(optimal_cache.sorted_records_of_cached_containers,
				(GCompareDataFunc)g_access_records_cmp_by_first_seqno, NULL);

		GSequenceIter *iter = g_sequence_iter_prev(
				g_sequence_get_end_iter(optimal_cache.sorted_records_of_cached_containers));
		struct accessRecords* r = (accessRecords*)g_sequence_get(iter);
		...
		while (i < 10 && iter != g_sequence_get_begin_iter(optimal_cache.sorted_records_of_cached_containers)) {
			...
			r = (accessRecords*)g_sequence_get(iter);
...
void* optimal_restore_thread(void *arg) {
	...
	struct chunk* c;
	while ((c = (chunk*)sync_queue_pop(restore_recipe_queue))) {
