...
void do_backup(char *path) {
    ...
    do{
        sleep(5);
        fprintf(stderr,"job %d, %lld bytes, %d chunks, %d files processed\r", 
                jcr.id, jcr.data_size, jcr.chunk_num, jcr.file_num);
    }while(jcr.status == JCR_STATUS_RUNNING || jcr.status != JCR_STATUS_DONE);
    fprintf(stderr,"job %d, %lld bytes, %d chunks, %d files processed\n", 
        jcr.id, jcr.data_size, jcr.chunk_num, jcr.file_num);
	...
	printf("number of chunks: %d (%lldbytes on average)\n", jcr.chunk_num,
