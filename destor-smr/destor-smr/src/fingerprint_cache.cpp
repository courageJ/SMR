...
int64_t fingerprint_cache_lookup(fingerprint *fp){
	switch(destor.index_category[1]){
		case INDEX_CATEGORY_PHYSICAL_LOCALITY:{
			struct containerMeta* cm = (containerMeta*)lru_cache_lookup(lru_queue, fp);
			if (cm)
				return cm->id;
			break;
		}
		case INDEX_CATEGORY_LOGICAL_LOCALITY:{
			struct segmentRecipe* sr = (segmentRecipe*)lru_cache_lookup(lru_queue, fp);
			if(sr){
				struct chunkPointer* cp = (chunkPointer*)g_hash_table_lookup(sr->kvpairs, fp);
	...
	switch(destor.index_category[1]){
		...
		case INDEX_CATEGORY_LOGICAL_LOCALITY:{
			if (!lru_cache_hits(lru_queue, &id,
				...
				while ((sr = (segmentRecipe*)g_queue_pop_tail(segments))) {
					/* From tail to head */
