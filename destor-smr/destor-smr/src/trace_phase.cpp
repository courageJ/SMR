...
void make_trace(char* path) {
	...
	*(p + 1) = 0;
	...
	while (1) {
		struct chunk *c = (chunk*)sync_queue_pop(hash_queue);
		...
		if (CHECK_CHUNK(c, CHUNK_FILE_START)) {
			destor_log(DESTOR_NOTICE, (const char*)c->data);
			fprintf(fp, "file start %zd\n", strlen((const char*)c->data));
			fprintf(fp, "%s\n", c->data);

		} else if (CHECK_CHUNK(c, CHUNK_FILE_END)) {
			fprintf(fp, "file end\n");
		} else {
			hash2code(c->fp, (char *)code);
			code[40] = 0;
			fprintf(fp, "%s %d\n", code, c->size);
		}
		free_chunk(c);
...
static void* read_trace_thread(void *argv) {
	...
	while (1) {
		...
		fgets((char *)c->data, filenamelen + 2, trace_file);
		...
		while (strncmp(line, "file end", 8) != 0) {
			...
			code2hash((unsigned char*)code, c->fp);
