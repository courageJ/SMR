...
#define MAX_CONTAINER_COUNT 8000 //buffer size
vector <chunk *> rewrite_buffer_chunk_pt; //chunk buffer
GHashTable *real_containerid_to_tmp; //container ID mapping
int64_t tmp_to_real_containerid[MAX_CONTAINER_COUNT];

vector <string> all_fp[MAX_CONTAINER_COUNT];//fingerprints of all useful chunks
bool is_container_selected[MAX_CONTAINER_COUNT]; // check whether the container is selected
vector <int> container_selected; //record the selected container ID
...
void *smr_rewrite(void* arg) {
	...
	while (1) {
		...
		if (!rewrite_buffer_push(c)) {
			...
			//store chunks of the backup segment into buffer
			TIMER_END(1, jcr.rewrite_time);
			continue;
		}

		//all containers IDs that chunks can refer to
		//add all chunks' fingerprints into the vector of the container they belongs to
        ...
        //if the number of referred containers is less than smr level, thus, the selected num is adjusted to the number of referred containers.
		...
		if (num > 0) {
			for (int subround = 0; subround < num; subround++) {
				...
				if (preDedup == -1) {
					break;//end early if there is no containers that can offer distinct useful chunks
				}
		...
		for (int chunk_id = 0; chunk_id < rewrite_buffer_chunk_pt.size(); chunk_id++) {
            ...
            //update the referred container ID for duplicate chunks
		}
